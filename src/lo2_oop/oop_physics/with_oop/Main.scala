...
import archived.physics.{PhysicsEngine, PhysicsVector, World}
...
object Main {
  ...
  def polymorphismExample(): Unit = {
    ...
    val ball1: DodgeBall = new DodgeBall(new PhysicsVector(-2.28, 4.88, 5.1689),
      new PhysicsVector(1.0, 1.0, 1.0), new PhysicsVector(-0.24, 8.59, -6.711), 2)

    val ball2: DodgeBall = new DodgeBall(new PhysicsVector(10.325, -2.14, 0.0),
      new PhysicsVector(1.2, 1.2, 1.2), new PhysicsVector(3.65, -9.0, -7.051), 5)

    val ball3: DodgeBall = new DodgeBall(new PhysicsVector(-6.988, 1.83, 2.5419),
