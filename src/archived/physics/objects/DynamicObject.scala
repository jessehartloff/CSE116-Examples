package archived.physics.objects
...
/**
  ...
  * to determine their movement, and mass to determine their momentum. The archived.physics engine will move these objects
  * during each update.
  *
...
class DynamicObject(location: PhysicsVector, dimensions: PhysicsVector) extends GameObject(location, dimensions) {
  ...
  /**
    * onGroundCalled has no real functionality, but will be used while testing the archived.physics engine. When testing
  ...
  /**
    ...
    * testing objective 3 of the archived.physics engine. When testing for collisions with static objects, use
  ...
  /**
    * previousLocation is used by the archived.physics engine and other objects to determine the movement vector of the
  ...
  /**
    * Called by the archived.physics engine whenever the object is on the ground. By default, an object does nothing when
    ...
    * by the archived.physics engine we can enable the jump button
    */
  def onGround(): Unit = {
  ...
  /**
    * Called by the archived.physics engine whenever the object collides with a static object. By default, an object does not
